...
from text2sql import asdl
import attr

from text2sql.util import ast_util
...
class SpiderLanguage(object):
    ...
    def unparse(self, tree, db, value_list):
        unparser = SpiderUnparser(self.ast_wrapper, db, value_list,
    ...
    COND_TYPES_F, COND_TYPES_B = bimap(
        range(0, 12),
        # Spider:
        ('Not', 'Between', 'Eq', 'Gt', 'Lt', 'Ge', 'Le', 'Ne', 'In', 'Like', 'Is', 'Exists'))
...
class SpiderUnparser:
    ...
    def refine_from(self, tree):
        ...
        Returns: bool -> indicates whether it is a row computation or not
        """
        ...
        if "from" in tree and tree["from"]["table_units"][0][
            ...
            return len(tree['from']['table_units']) == 2  # is a row computation
        ...
        for table_id in candidate_table_ids[1:]:
            ...
            for source_table_id, target_table_id in zip(path, path[1:]):
                ...
                conds.append({
                    '_type': 'Eq',
                    'agg_id': {
                        '_type': SpiderLanguage.AGG_TYPES_F[0]
                    },
                    'val_unit': {
                        '_type': 'Column',
                        'col_unit1': {
...
if __name__ == "__main__":
    ...
    spider_lang = SpiderLanguage(str(path))
    print("Finished")