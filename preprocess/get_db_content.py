...
def get_db_dict_content(db_path):
    ...
    for table in tables:
        ...
        content_dict[table] = [[str(cell) for cell in row] for row in rows]
...
def dump_db_json_content(data: datasets.Dataset):
    ...
    for idx, db_id in enumerate(tqdm(db_ids)):
        ...
        db_dct = get_db_dict_content(sql_db_path)
        json_db_content_path = db_path + "/" + db_id + "/" + db_id + "_content.json"

        if not os.path.isfile(json_db_content_path):
