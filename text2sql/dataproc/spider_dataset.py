...
class SpiderDataset(Dataset):
    ...
    def __init__(self, name, input_encoder, label_encoder,
        ...
        spider_data_dict = datasets.load_dataset(path="dataproc/loaders/spider.py", cache_dir=DATASETS_PATH, split=name)
        db_path = spider_data_dict["db_path"][0]
        ...
        if is_cached:
            ...
            # if not train_dct_cached:
            #
            #     with open(str(train_dct_path), 'wb') as outp:
            #         pickle.dump(self.db_dict, outp)
            # else:
            #     with open(str(train_dct_path), 'rb') as inp:
            #         print(str(train_dct_path))
            #         self.db_dict = dill.load(inp)
            self._examples = []
    ...
    def load(self, db_file, data_file):
        ...
        with open(data_file, 'rb') as ifs:
            sys.modules['util'] = sys.modules['text2sql.util']
            unpickler = CustomUnpickler(ifs)
...
if __name__ == '__main__':
    from text2sql.dataproc.bert_encoder import BertInputEncoder
    from text2sql.global_config import get_config
    ...
    #                            is_cached=False)
    # spider_ds = SpiderDataset('validation', model_config, label_encoder, train_dct_cached=True)
