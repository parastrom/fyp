...
from text2sql.dataproc import sql_preproc
...
def maybe_stack(items, axis=None):
    ...
    if not to_stack:
        ...
        return torch.stack(to_stack, axis)
...
class Text2SQLDecoder(torch.nn.Module):
    """Decoder model"""
    ...
    def __init__(
        ...
        self.state_update = torch.nn.LSTMCell(
        ...
        # TODO batching
        self.zero_rule_emb = torch.zeros([1, self.rule_emb_size], device='cuda:0')
        self.zero_recurrent_emb = torch.zeros([1, self.recurrent_size], device='cuda:0')
    ...
    def _index(self, vocab, word):
        """get token id"""
        return self._tensor([vocab.index(word)], device='cuda:0')
    ...
    def rule_infer(self, node_type, rule_logits):
        """rule infer"""
        rule_logprobs = torch.log_softmax(rule_logits, dim=-1)
    ...
    def gen_token_loss(self, output, gen_logodds, token, desc_enc):
        ...
        loss_piece = -torch.logsumexp(
            maybe_stack(
                [copy_logprob, gen_logprob], dim=1), dim=1)
        return loss_piece
    ...
    def token_infer(self, output, gen_logodds, desc_enc):
        ...
        # log p(copy | output)
        # shape: batch (=1)
        copy_logprob = F.logsigmoid(-gen_logodds)
        copy_loc_logits = self.copy_pointer(output, desc_enc.memory)
        # log p(loc_i | copy, output)
        # shape: batch (=1) x seq length
        copy_loc_logprobs = torch.log_softmax(
        ...
        # log p(~copy | output)
        # shape: batch (=1)
        gen_logprob = F.logsigmoid(gen_logodds)
        token_logits = self.terminal_logits(output)
        # log p(v | ~copy, output)
