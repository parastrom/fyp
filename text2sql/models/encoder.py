...
from text2sql.util import nn_util
from text2sql.util import linking
...
class Text2SQLEncoder(nn.Module):
    ...
    def __init__(self, model_config, extra=None):
        ...
        self.encs_update = relational_encoder.RelationAwareEncoder(
            ...
            num_relations=len(linking.RELATIONS),
    ...
    def forward(self, inputs):
        ...
        question_encs = nn_util.batch_gather_2d(seq_hidden,
                                                 question_tokens_index)
        table_encs = nn_util.batch_gather_2d(seq_hidden, table_indexes)
        column_encs = nn_util.batch_gather_2d(seq_hidden, column_indexes)
        value_encs = nn_util.batch_gather_2d(seq_hidden, value_indexes)
        ...
        for batch_idx, orig_input in enumerate(orig_inputs):
            ...
            enc_results.append(
                EncoderState(
                    ...
                    schema_memory=torch.cat(schema_memory, dim=1),
    ...
    def span_encoder(self,
        ...
        span_enc1 = nn_util.batch_gather_2d(seq_hidden, span_index)

        token_gather_index = torch.reshape(
            span_tokens_index, shape=[-1, max_col_nums * max_col_tokens])
        span_tokens_enc_origin = nn_util.batch_gather_2d(seq_hidden,
                                                          token_gather_index)

        span_tokens_weight = torch.reshape(
            torch.matmul(span_tokens_enc_origin,
                         cls_hidden.unsqueeze(-1)),
            [-1, max_col_nums, max_col_tokens])
        span_tokens_weight = F.softmax(
            nn_util.sequence_mask(span_tokens_weight, span_tokens_mask),
            dim=-1)

        span_tokens_enc_origin = torch.reshape(
            span_tokens_enc_origin,
            [-1, max_col_nums, max_col_tokens, hidden_size])
        span_enc2 = torch.sum(torch.multiply(
            span_tokens_enc_origin, span_tokens_weight.unsqueeze(-1)),
            dim=2)

        span_enc = torch.cat([span_enc1, span_enc2], dim=-1)
...
if __name__ == "__main__":
    ...
    inputs = {
        ...
        'question_tokens_index': torch.tensor(
            list(range(1, 5)), dtype=torch.int64).reshape([1, 4]),
        'column_index': torch.tensor(
            [1, 4], dtype=torch.int64).reshape([1, 2]),
        'column_mask': torch.tensor(
            [1, 1], dtype=torch.float32).reshape([1, 2]),
        'column_tokens_index': torch.tensor(
            [1, 2, 3, 4, 5, 0], dtype=torch.int64).reshape([1, 2, 3]),
        'column_tokens_mask': torch.tensor(
            [1, 1, 1, 1, 1, 0], dtype=torch.float32).reshape([1, 2, 3]),
        'table_index': torch.tensor(
            [1, 4], dtype=torch.int64).reshape([1, 2]),
        'table_mask': torch.tensor(
            [1, 1], dtype=torch.float32).reshape([1, 2]),
        'table_tokens_index': torch.tensor(
            [1, 2, 3, 4, 5, 0], dtype=torch.int64).reshape([1, 2, 3]),
        'table_tokens_mask': torch.tensor(
            [1, 1, 1, 1, 1, 0], dtype=torch.float32).reshape([1, 2, 3]),
        'limit_nums_index': torch.tensor(
            [1, 4], dtype=torch.int64).reshape([1, 2]),
        'limit_nums_mask': torch.tensor(
            [1, 1], dtype=torch.float32).reshape([1, 2]),
        'orig_inputs': [{
            'columns': ['a', 'b'],
            'tables': ['t1', 't2'],
